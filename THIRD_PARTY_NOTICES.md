...
## 설계 참고 (코드 미포함) — 09-03 신설

다음 오픈소스 프로젝트와 표준 문서의 **공개 문서·데모**를 **설계 참고 자료**로 사용했다.
**코드·에셋·스타일 값은 포함하지 않았고, 모든 구현은 독자적으로 작성했다.**

| 참고한 것 | 라이선스 | 무엇을 배웠나 |
|---|---|---|
| driver.js | MIT | 말풍선이 뷰포트를 벗어나면 반대편으로 뒤집는다는 개념 |
| Reactour | MIT | 스포트라이트 여백을 값으로 노출한다는 개념 |
| react-joyride | MIT | 단계 정의를 선언으로 두고 런타임이 해석한다는 개념 |
| react-guided-journey | MIT | 진행률 점 표시 |
| tour-kit 코어 3본(`@tour-kit/core`·`react`·`hints`) | MIT | 힌트와 투어를 다른 컴포넌트로 가르는 구성 |
| WAI-ARIA Authoring Practices | W3C 문서 | Dialog 패턴의 이름·역할·포커스 규정 |
| WCAG 2.2 (2.4.11·2.4.12·2.5.5·2.5.8) | W3C 권고 | 합격선(AA)과 목표(AAA)를 다른 수준으로 두는 것 |

🔴 **이 표는 «가져온 것»의 목록이 아니라 «본 것»의 목록이다.** 가져온 코드는 **0**이므로 저작권 고지 의무는 발생하지 않는다 — 그럼에도 적는 이유는, **정직함이 곧 방어 근거**이기 때문이다(관찰→명세→구현 3단 절차와 그 산출물 = `docs/tour/technique-notes.md`).

**참고하지 않기로 한 것** — intro.js·Shepherd.js(**AGPL**) · GuideChimp(**EUPL-1.2/상용 이중**) · 이름만 알려진 「Guide.js」(**403 으로 열리지 않아 목록에서 제외**) · tour-kit 의 **상용** 패키지군. 🔴 `usertour.io` 는 `usertourkit.com` 과 **다른 회사**다.
