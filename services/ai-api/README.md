...
| `run_cap_global_per_hour` | **3** | 같은 파일(`FKT_RUN_CAP_GLOBAL_PER_HOUR` · 창 3600 고정 · 0 이하 = 끄기 · 🔴 **프로세스 단위** — 재기동하면 창이 리셋되고 워커가 여럿이면 워커별로 센다) |
...
호출 수는 `step.completed(synthesize).payload.synthesis` 의 **`calls`**(1|2)와
