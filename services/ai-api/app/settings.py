...
class Settings(BaseSettings):
    ...
    #    🔴 **5 → 3**(계약 v0.2.4 · 운영자 하명 09-11). 앞 개정(3→5)의 사유는 「기본값은
    #       가장 자주 읽히는 문서와 같아야 한다」였고 **그 사유는 지금도 참이다** — 바뀐 것은
    #       문서 쪽이다(README·runbook·ai-api README 의 「5회」가 전부 3 으로 간다). 즉 이 줄은
    #       그 원칙을 뒤집는 것이 아니라 **같은 원칙을 새 문서값에 다시 맞추는 것**이다.
    #       env 우선순위는 그대로다(env 가 있으면 env).
    run_cap_per_session: int = 3
    run_cap_window_sec: float = 3600.0

    # 🔴 **전역 시간당 Live 상한**(계약 v0.2.4 ① ⓑ · 신설). 세션 축과 «다른 것»을 막는다:
    #    세션 상한은 한 방문자의 소모를, 이것은 **구독 전체의 소모**를 막는다. 창 3600 고정.
    #    0 이하 = 끄기(세션 축과 같은 규칙 · 「0 이면 아무도 못 돈다」로 두지 않는다).
    # 🔴 **프로세스 단위다** — 재기동하면 창이 0 으로 리셋되고, 워커가 여럿이면 워커별로
    #    센다. 지금 형상(컨테이너 1본·단일 프로세스)에서는 전역과 동치지만, 그것은 **배치가
    #    그래서**이지 이 코드가 분산 정확성을 주는 것이 아니다 — 주장하지 않는다.
    run_cap_global_per_hour: int = 3
