...
- 🔴 **checks 대기 루프는 「아직 안 끝났다」와 「없다」를 가른다**(09-22 63대 자수) — 종료 조건을 「전부 terminal」로만 잡으면 **목록이 비었을 때 조용히 영원히 돌다**(런 미생성·조회 실패). 「부재는 초록이 아니라 판정 불가」 — 계수 0 이면 통지하고 중단한다. 같은 이유로 **계기의 침묵을 상태로 읽지 않는다** — 통지가 안 오면 실물을 1회 직독한다.
- 🔴 **`timeout N <cmd>` 등 네이티브 프로세스 트리에 그룹 신호를 보내는 대기 금지 · 예외 0**(D-99 계기 · 09-22 · 만료가 쏘는 그룹 신호가 대상 밖에 닿아 production Docker 가 2일 3시간 멈춘 자리) — 대체 = 단발 확인 + 다음 단위 · Monitor · 「종료 조건에서 스스로 끝나는」 background 폴링 루프. `--foreground` 를 예외로 뚫지 않는다 — 「안 쓴다」가 아니라 「다르게 쓴다」이고 그 차이를 재 본 적이 없다(미측을 예외의 근거로 쓰지 않는다).
- **Windows 에서 `git worktree remove` 는 `node_modules`·`.next` 잔여로 「Directory not empty」 실패 후 등록만 해제될 수 있다** — 디렉토리 삭제(`Remove-Item -Recurse -Force`) 뒤 `git worktree prune` 까지가 정리이고, 확인은 목록이 아니라 **경로 실재**로 한다(09-04 23:58 리바이2 실측). 제거 전 3단 대조 = 병합 실측(`git rev-list --count origin/develop..lane` 0) · Mounts.Source ∩ `_wt` 0 · **실행 프로세스 cwd 가 그 워크트리 안이면 제외 — cwd 는 `psutil` 로 pid 별 직독(명령줄·exe 는 보조 · 09-05 21:33 리바이2 실측: 서버 5대가 명령줄·exe 0건 · cwd 5건 = 명령줄만 봤으면 삭제) · cwd 판독 불가 pid 가 개발 도구면 그 트리 보류**(무대 `:8199` 보존 사례). cwd 스캔은 **자기 계보(self+조상 pid)를 배제**하고 **2.5초 간격 2표본의 교집합**만 「산 것」으로 센다 — 스캐너 자신의 셸이 그 트리를 cwd 로 잡아 거짓 HOLD 를 낸 자리(09-05 21:58 리바이2 실측). 🔴 **cwd 축 보강 2(09-06 07:25 센쿠2 51대 실증 · 스캔 3회)**: ⓐ **background 작업 셸은 「자기 계보」 밖이다** — 자기 세션이 띄운 background 폴링(CI 대기 등)은 self+조상 체인에 없어 거짓 HOLD 를 낸다 → 스캔 «전»에 자기 background 작업을 먼저 끝낸다. ⓑ **TaskStop 성공 ≠ 프로세스 종료** — 부모만 죽고 자식(`gh.exe` 등)이 고아로 같은 cwd 를 붙잡는다 → 중단 뒤 재스캔에서 조상 없는 고아를 pid 로 찾아 죽인 뒤에야 clear 다. 어느 쪽이든 1·2회차 HOLD 를 그대로 믿으면 근거 없는 보류가 된다. 🔴 **④ 미커밋 축(09-06 01:45 센쿠2 회부)**: ①②③은 커밋·컨테이너·프로세스만 본다 — 미커밋 작업물은 세 그물 사이로 빠진다. 제거 전 `git -C <트리> status --porcelain` 계수를 대조에 넣고, 0 이 아니면 **목록을 먼저 찍은 뒤** 제거하거나 보류한다. `git worktree remove` 가 `contains modified or untracked files` 로 거부한 것을 `Remove-Item -Force` 로 넘기면 「무엇을 잃었는지 말할 수 없는」 자리가 된다(09-06 5본 실증). 🔴 **⑤ 거부 뒤 잔여(09-06 01:55 센쿠2·리바이2 회부 3본 실증)**: `worktree remove` 가 `Directory not empty` 로 거부해도 git 은 **등록·`.git` 을 이미 해제**한다 — 그 디렉터리는 ④가 영원히 0 이고 목록에도 없다(「목록에서 사라짐 ≠ 정리됨」). 판정은 **remove «전» 4단 값**으로 하고, 거부 뒤 잔여는 등록 해제·`.git` 부재를 확인한 «잔여 산물»로 분류해 오케가 삭제를 승인한다(`Remove-Item -Recurse -Force <경로 문자열 고정>` → `prune` → 경로 실재 없음). 좌석은 자기 판단으로 `-Force` 를 쓰지 않는다. **`--ignored` 는 remove «전»에 잰다 — 거부 뒤에는 그 축도 죽는다**(센쿠2 01:58 · `ci-unit` 794M 의 ignored 잔여 = 「모른다」로 남은 자리).
