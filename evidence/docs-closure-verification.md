...
## 9. 재검 — 회부 3건 수리분 (`lane/suzaku-60-docs` `03dbf29` · `date 00:1x` 09-12)

수리 범위 = 3파일 3줄(`git diff 147995d 03dbf29 --stat` 으로 확인 · 그 밖의 변경 0).

| 회부 | 수리된 줄 | 내 대조 | 결과 |
|---|---|---|---|
| 1 · README 표 | `API 계약 74/74 (09-04) → 99/99 (09-11 · 계약 v0.2.5 · 커버리지 63/63)` | **두 시대를 각각 셌다**: 09-04 당시 승격 14 `879fc35` 의 케이스 파일 = **74** · `03dbf29` = **99** · 러너 재실행 = **99/99 · 자기 검증 PASS · 커버리지 63/63** | 🟢 **해소** |
| 2 · 원장 머리글 | 「T5-2 완결 09-11 23:10(#1004·#1005 …)」 선행 + 09-06 은 「1차 판정」으로 보존 | 11개 축 전건 일치(수치 PASS 11·조건부 1·구조적 면역 1·정적 감사 1·FAIL 0 · `evidence/t5-2-gate7-close.md` 경로 · 완결 절이 1차 절보다 앞) · **23:10 = #1005 병합 실측 23:10:47** | 🟢 **해소** |
| 3 · runbook | 「production 스택 3본(… 네트워크·DSN 귀속)」 | `docker network inspect fkt-senku2-t15_default` 소속 컨테이너 = **정확히 3본**(`fkt-deploy-ai-api` · `…postgres-1`(별칭 `postgres`) · `…neo4j-1`(별칭 `neo4j`)) | 🟢 **해소** |

🔴 **74 는 「그때 그랬다」는 과거 주장이라 오늘 러너로는 못 센다** — 그 시대의 케이스 파일을 꺼내 세어야 값이 된다.
그래서 두 시대를 각각 세었고, 오늘 값 하나로 과거 값을 추정하지 않았다.

🔴 **러너 재실행의 귀속**: 내 트리의 `tests/contract`·`packages/contracts` 는 `03dbf29` 와 `git diff` **0**이다.
그래서 내 실행 결과를 수리본의 값으로 쓸 수 있다 — 다른 트리에서 돌린 숫자를 그냥 옮긴 것이 아니다.

**재검 판정 = 회부 3건 전건 해소.** 수리가 건드리지 않은 축(①②④⑤)은 재측하지 않았다(3줄 밖 변경 0 확인).
