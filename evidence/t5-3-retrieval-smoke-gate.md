...
- 🔴 **exit code 분리**: 구조 가드 = **exit 3~7**(「측정 성립」 — 표본<8·서버 미도달·전체 hit0·교정 대조군 hit·**색인 없음**) · 회귀 게이트 = **exit 10**(「기준선 아래로 하락」). 둘을 한 코드로 섞지 않는다 —

  - 🔴 **exit 5 이분(O-54 · 09-06)**: 「answerable 전체 hit 0」은 두 가지 서로 다른 사실을 한 색으로 덮고 있었다.
    · **exit 7 = 색인 없음** — 전 호출이 비200 으로 «일관»하고 그 `error.code` 가 부재 목록(`index_unavailable`)에 있을 때.
    · **exit 5 = 검색이 죽었다** — 그 밖의 전부(200 위의 hit 0 · 부재가 아닌 5xx · 상태코드가 섞임).
    셋을 모두 요구하는 이유는 하나라도 어긋나면 원인이 섞인 것이라 「색인 없음」이라 부를 수 없기 때문이다.
    갈래가 실제로 갈리는지는 `benchmarks/tools/smoke-runner-branch-drill.mjs` 가 8칸으로 묻는다(반쪽 스텁 · 구독 0).
