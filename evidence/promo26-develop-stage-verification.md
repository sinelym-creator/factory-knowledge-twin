...
| ④ D-87 4종 404 | **PASS** | E1 |
| ⑤ 화면 배지 렌더 | 🔴 **미측** — 주어가 둘로 좁혀졌다(5·5-1절) | E1 |
| ⑥ CI | **PASS** | E1 |
...
### 5-1. 두 번째 시도 — 가설을 죽였고, 주어가 좁아졌다 (E1)

MSYS 경로 변환을 고친 뒤 표지를 심은 fixture 로 다시 쟀다. 30초 동안 **0.5초 간격 폴링**으로 흐르는 창을 지켜봤다(완주 뒤 진입은 스냅샷이라 이벤트를 못 볼 수 있으므로).

| 물음 | 실측 |
|---|---|
| 대상이 이벤트를 받았는가 | **예** — `RUN-a3c309355896` 이벤트에 `evidence.flagged` **1건**(items 2) |
| 화면이 흐르는 창을 열었는가 | **예** — 무대 로그에 `WebSocket /api/ws/runs/RUN-72fe3f1ef8e9 [accepted]` |
| 카드 id 와 심은 id 가 같은가 | **예** — 화면 카드 id 에 `EQ-CNC-204`·`SN-204-VIB` **둘 다 실재** |
| 배지가 그려졌는가 | **아니오** — 30초 폴링 내내 **0** |

🔴 **「내가 심은 id 가 카드 id 와 다르다」는 내 가설은 반증됐다.** 세 전제가 모두 참인데 배지가 없다.

🔴 **그래도 「배지가 안 그려진다」로 회부하지 않는다** — 마지막 한 칸이 비어 있다: **그 `evidence.flagged` 가 WS 프레임으로 실제 흘렀는가**. 서버가 REST 이벤트 목록에는 싣고 WS 로는 안 보냈을 수 있고, 그러면 주어는 화면이 아니라 전송 층이다. 그 프레임을 뜯지 못했으므로 **미측**이고, 주어는 「화면 렌더」와 「replay WS 전송」 둘 중 하나로 좁혀진 상태다.
