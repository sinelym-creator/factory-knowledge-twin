...
| ⑥ CI | **PASS** | E1 |
| ⑦ 투어 4건 | 🔴 **E-4 FAIL(수리 회부)** · 나머지 **PASS** | E1 |
...
- **미측** ⑤ 화면 배지 브라우저 실렌더(5절) — 🔴 **자수 11**: 표지를 심은 fixture 사본을 서버가 `replay_fixture_missing` 으로 거절했다. 마운트 실재만 확인하고 **서버가 그 파일을 받아들였는지**는 확인하지 않은 채 화면부터 열었다. 자극은 「심었다」가 아니라 **「대상이 받았다」**로 확인해야 한다.
