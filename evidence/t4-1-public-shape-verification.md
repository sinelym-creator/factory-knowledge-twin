...
# T4-1 단축 재검 — **3행 + 비차단 1행** (14대)

> 검증 좌석 리바이2 **14대** · lane `lane/levi2-t4-1-recheck` @ 기점 `4596407`(13대 판정본 **아래**
> append · rebase 없음) · 픽스 대상 = 「부팅 강건성 소조각」 PR#222 `8b686cc`(develop 조상 확인) ·
> 근거 **E1**(전건 실측) · 측정 2026-08-31 18:18~20:0x KST.
>
> **판정: ① D-2 PASS · ③ Q-52 PASS · ② D-3 = FAIL(재개방 · 등급 중 · T4-1 완결 차단)**
> · 비차단 ④(Q-37·D-4 중단 창) = 측정 성립 — 갈렸던 두 주장이 **각각 반씩** 참이다.

## §R0 계측기 — 이번에도 내 것부터 (4건)

| # | 무엇이 거짓말했나 | 어떻게 드러났나 | 교정 |
|---|---|---|---|
| 1 | ② 대조군을 `route.abort()` 로 세웠다 | 입장이 «실패»로 끝나며 **셸·배지가 통째로 사라졌다**(mode=null · 821ms). 대조군이 **대상을 없애 버려** 아무것도 못 갈랐다 | `route` 를 **지연**(55s)으로 바꿔 화면은 `/` 에 두고 자극만 계속 돌게 했다 |
| 2 | ③ 회복 폴러가 첫 표본을 +8.8s 에 냈다 | 회복 «순간»의 연속 자취가 비었다 — 원인 **미규명** | 판정은 전/후 쌍 + 8057 BEFORE 대조군에 세우고, 연속 자취는 **없다고 적는다**(잰 범위 밖 결론 금지) |
| 3 | ④ TCP 탐침 뒤에 HTTP 탐침을 **같은 루프**에 붙였다 | TCP 성공 표본이 10건 → **1건**으로 줄었다. 내 HTTP 호출이 루프를 멈춰 **내 표본을 스스로 갉아먹었다** | HTTP 축을 **별 프로세스**로 분리해 아무것도 앞세우지 않고 다시 쟀다 |
| 4 | ④ 첫 판에서 25초 창에 「TCP 0 · 종료 안 함」이 나왔다 | 같은 조건 3회 재실행은 전건 **exit=1 · Ready 관측**. 첫 판은 부하로 `Ready` 자체가 25초를 넘긴 것이고, 그것을 「리슨 안 한다」로 읽을 뻔했다 | 창을 60s 로 늘려 3회 재현 후 판정 |

### §R0.1 측정 조건

- 🔴 **새 프로젝트 · 새 볼륨**: `COMPOSE_PROJECT_NAME=fkt-levi2-t41r` · pg **5560** · neo4j **7610/7620** ·
  ai-api **8071** · `VOLUME_ROOT=./.volumes-t41r` · `FKT_BUILD_SHA=4596407`.
  기존 프로젝트명 재-`up` **금지 규율 준수** — 존치 스택 5벌·셸 3151 전건 무접촉.
- 워크트리 `_wt/levi2-t41r`(lane 기점 `4596407`) · 블랙홀 셸 **:3171**(빌드 `FKT_API_BASE=8074`).
- 회귀 조건 4줄 전건 적용 · 로그 인용 절대경로 마스킹(Q-49).

---

## §R1 ① D-2 — 「새 프로젝트·새 볼륨 첫 부팅이 예산 안에 선다」 = **PASS**

🔴 **예산의 낱말이 바뀌었다 — 두 자를 나란히 적는다**(«바뀜»이 아니라 성문 갱신):

| 시점 | pg healthcheck 예산 |
|---|---|
| 13대 측정 당시 | `interval 5s x retries 12` = **60s** · `start_period` **없음** |
| 지금(PR#222) | `start_period` **40s** + `interval 5s x retries 12` · 데이터 자리 = **named volume `pgdata`** |

| 무엇 | 실측 |
|---|---|
| 볼륨 | `Volume fkt-levi2-t41r_pgdata Creating -> Created` — **새 볼륨이 실재로 생겼다** |
| `up -d` | **rc=0 · 72,743ms** |
| 🔴 pg 정본(컨테이너 로그) | `StartedAt 09:25:06.107` -> `database system is ready to accept connections` **09:25:07.837** = **1.73s** |
| pg healthcheck | `healthy` · **FailingStreak=0** — 한 번도 실패로 세지 않았다 |
| 72.7s 의 몸통 | **neo4j** — `Started.` 09:25:57.28(약 +51s). pg 는 예산의 **2.9%** 를 썼다(1.73s / 60s) |
| ai-api | `StartedAt 09:26:07.08` -> healthy |

**「생겼다 ≠ 선다」 네 곳 전부:**

| 층 | 실측 |
|---|---|
| compose ps | ai-api·neo4j·postgres 전건 `running / healthy` |
| 포트 리슨 | 5560 · 7610 · 7620 · **8071** 전건 `LISTEN` |
| HTTP | `GET /api/health` **200** · 0.05s |
| 본문 | `status:"ok"` · postgres ok · neo4j ok · **`build:"4596407"`** · embedding ready |

⇒ 13대 ② 행(bind 32.8s)과 ③ 행(named 1.76s)의 **대조가 픽스로 굳었다** — 이번 새 볼륨 첫 부팅은
**1.73s** 로 named 대조군과 같은 자리다. **D-2 = 해소.**

---

## §R2 ③ Q-52 — 「기동 역전 뒤 pg 회복 시 `postgres.state` 가 ok 로 돌아온다」 = **PASS**

### 2.1 자극이 실재했는가

| 무엇 | 실측 |
|---|---|
| 자극 | `docker compose stop postgres` -> `up -d --no-deps --force-recreate ai-api`(pg 없는 동안 기동) |
| 자극 후 3연속 | `postgres.state = unavailable` · `detail "TimeoutError"` · `status:"degraded"` |
| 🔴 상한이 실물로 보인다 | 그 3건 중 하나의 응답이 **2.009s** = `settings.probe_timeout_sec = 2.0`(E2) |

### 2.2 회복

| 회차 | pg 회복 -> `postgres.state = ok` |
|---|---|
| 1회 | **+18.8s**(pg health=healthy) |
| 2회 | **+8.8s**(응답 0.138s) |
| 최종 | `status:"ok"` · postgres ok(1ms) · neo4j ok |

### 2.3 🔴 이 초록의 뜻은 대조군이 준다

**BEFORE 표본 = 13대 존치 스택 `t41y`(8057) — 읽기만 하고 손대지 않았다.** 그 서버는 **지금도**
`postgres.state = unavailable`(TimeoutError)이다 — 자기 pg 는 몇 시간째 healthy 인데도.
「pg 가 돌아오면 서버도 돌아온다」가 **원래는 참이 아니었다**는 사실이 그 표본에 살아 있고,
그래서 위 초록이 「원래 그랬던 것」이 아니라 **픽스의 것**임이 갈린다.

⚠ **자수**: 회복 «순간»의 연속 자취는 못 남겼다(§R0 자수 2). 판정은 **자극 후 3연속 unavailable
-> 회복 후 ok** 라는 전/후 쌍과 위 BEFORE 대조군에 선다. 「몇 초 만에 돌아오는가」는 두 표본
(18.8s · 8.8s)뿐이며 분포로 주장하지 않는다.

---

## §R3 ② D-3 — 「배지가 상한 + e 안에 «미연결»을 말한다」 = 🔴 **FAIL (재개방)**

> 오케 15:26 판정은 「D-3 = Q-50 병합 · 13대 소견 기각 후보 · 검증 확인 1행」이었다.
> **확인해 보니 닫히지 않는다.** 두 주장이 **모두** 내 실측과 어긋난다.

그물 = `tests/web/q50_entry_badge_timing.mjs`(신규) · 자극 = 블랙홀 `:8074` · 셸 `:3171`(그 값으로 빌드).

### 3.1 본 경로 — 3회 재현

| 무엇 | 1회 | 2회 | 3회 |
|---|---|---|---|
| ⓐ `/api/live/status` **끊긴 간격**(앱 상한 2s) | 2001·2004ms | 2013·2009ms | 2005·2011ms |
| ⓒ `POST /enter` | 2310ms | 2150ms | 2055ms |
| ⓒ `/overview` SSR | **2533ms** | **948ms** | **3645ms** |
| ⓑ 배지 «미연결» + 정적 제안 | **12,972ms** | **12,827ms** | **12,489ms** |

🔴 **SSR 이 0.95초여도 총합은 12.5초다.** 센쿠2 산식 「12.4s = 입장 2s + SSR **8s** 직렬 + tick 2s」는
이 조건에서 **성립하지 않는다** — SSR 이 2.7배 흔들려도 배지 시각은 +-0.5초 안에 고정이다.

자취가 말하는 것(3회 동형):

```
   352 ms  REQ  GET  /api/live/status      <- 첫 tick 시작
  2357 ms  CUT  ERR_ABORTED               <- 🔴 실패를 «2.4초에 이미 알았다»
  2408 ms  RES  303  /enter  ->  /overview
  6053 ms  RES  200  /overview            <- SSR 끝
 10476 ms  REQ  GET  /api/live/status      <- 두 번째 tick(= /overview 마운트의 첫 tick)
 12487 ms  CUT  ERR_ABORTED
 12489 ms  배지 «미연결» + 정적 제안 등장   <- 여기서야 바뀐다
 40481 ms  REQ  GET  /api/live/status      <- 10476 + 30000 (POLL_MS 와 정확히 일치)
```

### 3.2 🔴 대조군 — 항해를 없애면(= `/` 에 묶어 두면)

`POST /enter` 를 **지연**시켜 브라우저를 `/` 에 둔 채 같은 자극을 준다.

| 무엇 | 실측 |
|---|---|
| `/api/live/status` 끊김 | **2003 · 2003 · 2016 ms** — 자극은 계속 돌고 전건 상한 안에 끊긴다 |
| 배지 | **50초 동안 `checking`(확인 중)** — 끝내 안 바뀐다 |
| 정적 replay 제안 | **끝내 안 뜬다** |

⇒ 「입장 `/` 경로에서 <=3s 에 미연결 + 정적 제안」(센쿠2 약 2.2s)은 **재현되지 않는다.**
⇒ 13대 소견 「첫 tick 결과가 버려진다」는 **기각되지 않는다.** `/` 에서는 두 번째·세 번째 tick 도
화면에 닿지 않는다.

### 3.3 무엇이 갈렸나 (E2 보강 · 픽스는 검증이 고르지 않는다)

`apps/web-console/components/live-status.tsx` — `POLL_MS = 30_000` · 마운트 시 1회 tick 후 30초 주기.
본 경로 3번째 요청이 `10476 + 30000 = 40481ms` 로 **정확히** 맞는다 — 폴러 자체는 산 것이다.
그럼에도 `/` 위의 배지는 그 tick 들의 실패를 한 번도 말하지 않았다.

### 3.4 왜 가볍지 않은가

정적 replay 제안은 **ai-api 가 없는 방문자에게 유일한 진입로**다(T4-2a). 그것이 입장 화면에서
뜨지 않고, 뜨더라도 **12.5초**가 걸린다면, 그 12.5초 동안 화면은 「확인 중」이라 말한다 —
**닿지 못한다는 사실을 아는 채로 모르는 척하는 10초**다. 13대 유언이 가리킨 바로 그 자리다.

**단축 재검(픽스 후 1행)**: `FKT_WEB_BASE=<블랙홀 셸> node tests/web/q50_entry_badge_timing.mjs` ->
본 경로 «미연결+제안» <= 상한+e **이고** 대조군(`/` 고정)도 같은 창 안.

---

## §R4 ④ 비차단 — Q-37 / D-4 «중단 창» = **측정 성립 · 두 주장이 반씩 참**

자극 = 빌드 값과 다른 런타임 값으로 `next start` · 100ms 격자.

| 축 | 실측 |
|---|---|
| 종료 | **exit=1 · 스스로** — 3/3(+1.3s · +2.3s · +5.6s · +6.2s) · stderr = 전용 중단 문구 |
| ⓐ **TCP accept** | **표본 실재** — 1~10건 · 창 0~1.6s(첫 판 `+4326ms -> +5945ms`) |
| ⓑ **HTTP 응답** | **0건** — HTTP 전용 프로세스로 3회 재측(100ms 격자) 전건 0 |

⇒ 센쿠2 「Ready 후 짧은 리슨」 = **참**(포트는 잠깐 accept 한다) ·
   13대 「100ms 폴링 표본 0」 = **HTTP 축에서 참**(그 창으로 화면은 안 나간다) ·
   두 문장은 **다른 층**을 말하고 있었다. 🔴 **잰 것은 여기까지다** — 100ms 격자보다 짧은 창에서
   HTTP 가 한 번 나갔을 가능성은 이 측정으로 배제되지 않는다(미측).

**D-4 픽스 확인**: `FKT_PUBLIC_HTTPS` 를 **런타임에만** 주면 boot-check 가 **exit=1** 로 죽이고
전용 문구(「빌드(정본): (없음) / 런타임: 1」)를 낸다 — 13대가 회부한 「가드 없음」이 **해소**됐다.
(그 판의 TCP 창은 11.4초였으나 Ready 자체가 11.5초 걸린 **부하 판**이라 창 길이는 주장하지 않는다.)

🔵 **작은 회부(등급 하)**: 중단 창 동안 **포트는 accept 하고 HTTP 는 안 나간다.** TCP 로만 살피는
감시가 있다면 그 창에서 셸을 «떠 있다»로 읽는다 — 지금 그런 감시는 없으니 결함이 아니라 기록이다.
...
| 행 | 판정 | 근거 |
|---|---|---|
| ① D-2 새 볼륨 첫 부팅 예산 | **PASS** | §R1 |
| ② D-3 배지 «미연결» <= 상한+e | 🔴 **FAIL — D-3 재개방(등급 중 · 완결 차단)** | §R3 |
| ③ Q-52 기동 역전 후 ok 복귀 | **PASS** | §R2 |
| ④ Q-37·D-4 중단 창(비차단) | 측정 성립 · D-4 가드 해소 확인 | §R4 |

**T4-1 = 여전히 조건부 PASS.** 완결 = **D-3 픽스 + 단축 재검 1행**(§R3 끝).
D-2·D-4 는 이 재검으로 **해소**됐고, Q-52 도 **해소**됐다(BEFORE 표본 대비 실증).

**존치·인계**: 재검 스택 `fkt-levi2-t41r`(8071 · 새 named volume) · 블랙홀 `:8074` · 셸 `:3171`(블랙홀 빌드) ·
T3-5 재검용 `:8061`·`:3161` — 전부 **살려 둔다**(D-3·D-5 픽스 착지 시 그대로 재사용).
13대 존치 5벌 + 셸 `:3151` + `t41y`(8057 = Q-52 BEFORE 표본) **무접촉**.

## §R6 유언 (14대 · 재검분)

> **「대조군이 대상을 없애면 그것은 대조군이 아니다.」**
> `/enter` 를 **막았더니** 셸이 통째로 사라졌고, 배지가 없는 화면에서 나는 아무것도 못 갈랐다.
> **지연**으로 바꾸자 비로소 「항해 때문인가, 배지 때문인가」가 한 판에 갈렸다.
> 자극을 설계할 때는 «무엇을 남겨 두어야 관측이 성립하는가»부터 적어라.

> **「두 사람이 다투면 대개 서로 다른 층을 말하고 있다.」**
> 「짧은 리슨이 있다」와 「표본 0이다」는 **둘 다 참**이었다 — 하나는 TCP, 하나는 HTTP.
> 갈린 주장을 만나면 어느 쪽이 옳은지 묻기 전에 **둘이 같은 것을 재고 있는지**부터 물어라.

---

## §R7 ②′ 단축 재검 — D-3 픽스 뒤 = **PASS** (검증 좌석 리바이2 **16대** · 2026-08-31 23:4x KST)

> 🔴 **한 변수만 바꿨다.** 자극(블랙홀 `:8074`) · 그물(`q50_entry_badge_timing.mjs`) · 워크트리
> (`_wt/levi2-t41r`) · 관측 방식 전부 §R3 그대로다. 바뀐 것은 **셸의 sha 하나**다.

### R7.1 측정 조건

| 무엇 | 값 |
|---|---|
| 대상 | develop **`684aa0a`**(= D-3 픽스 PR#242 `831af7a` 포함 · `merge-base --is-ancestor` 실측) |
| 픽스가 손댄 자리 | `apps/web-console/` 4파일(`app/enter/route.ts` · `components/enter-form.tsx` · `lib/contract.ts` · `lib/session.ts` · +95/-4) |
| 셸 | `:3171` · `FKT_API_BASE=http://127.0.0.1:8074` 로 **production build**(Q-37) · pid 203460 |
| 🔴 한 변수 보증 | 빌드 산출물 `.next/server` 를 훑어 구워진 주소가 **`127.0.0.1:8074`** 임을 실측(BUILD_ID 23:38:10) |
| 🔴 자극 실재 | `curl -m 5` → `connect=1.5ms` · `total=5.01s` · rc=28 ⇒ **accept 하고 안 답한다**(거부가 아니다) |
| BEFORE | §R3 값 인용(재측 안 함) — 본 경로 **12,489~12,972ms**(3회) · `/` 묶음 **50초 checking** |

### R7.2 본 경로 3회 — **3/3 상한 + ε**

| 무엇 | 1회 | 2회 | 3회 |
|---|---|---|---|
| ⓐ `/api/live/status` **끊긴 간격**(= 앱 상한 실측) | 2,007ms | 2,004ms | 2,009ms |
| ⓑ 배지 «미연결» + 정적 제안 최초 | **2,173ms** | **2,295ms** | **2,245ms** |
| 🔴 그때의 **URL** | **`/`** | **`/`** | **`/`** |
| **ε**(= ⓑ − ⓐ) | **166ms** | **291ms** | **236ms** |
| 🔴 `checking` 중 제안 | **0회**(전이가 «확인 중 → 미연결» **2건뿐**) | 0회 | 0회 |
| ⓒ `POST /enter` | 2,053ms | 2,016ms | 2,038ms |
| ⓒ `/overview` SSR | 44ms | 5ms | 6ms |
| 빈 화면 아님(글자수) | 222 | 222 | 222 |

**자취(1회 · 3회 동형)** — §R3.1 의 같은 자리와 맞대어 읽어라.

```
    6 ms  REQ  GET   /
  158 ms  REQ  POST  /enter                <- 입장이 «첫 tick 과 나란히» 나간다
  158 ms  REQ  GET   /api/live/status
 2165 ms  CUT  net::ERR_ABORTED            <- 실패를 2.2초에 알고
 2173 ms  배지 «미연결» + 정적 제안          <- 🔴 «그 자리에서» 말한다 (url = /)
 2211 ms  RES  303   /enter -> /overview   <- 항해는 그 «뒤»다
```

🔴 **주어가 바뀌었다.** §R3 에서는 실패를 2.4초에 알고도 배지가 12.5초까지 침묵했다 —
첫 tick 의 결과가 항해에 삼켜졌기 때문이다. 지금은 **항해보다 38ms 먼저** 배지가 바뀐다.
「첫 tick 결과가 버려진다」(13대 소견)는 **처방으로 닫혔다**.

### R7.3 🔴 대조군 — `/` 에 묶어 두면 (§R3.2 와 같은 자극)

| 무엇 | 1회 | 2회 | 3회 |
|---|---|---|---|
| 끊긴 간격 | 2,005ms | 2,013ms | 1,999ms |
| 배지 «미연결» + 제안 | **2,212ms** | **2,186ms** | **2,250ms** |

⇒ BEFORE 의 **「50초 동안 `checking` · 제안 끝내 안 뜸」이 사라졌다.**
🔴 이 대조군이 판정의 절반이다 — 본 경로만 빨라졌다면 「항해가 짧아졌을 뿐」일 수 있는데,
**항해가 아예 없는 자리에서도** 상한+ε 안에 바뀌므로 고쳐진 것은 **배지가 tick 을 듣는 방식**이다.

### R7.4 `t41_live_status_timeout` 1회

상한 **2,004ms** 실재 ○ · 배지 «미연결» **2,270ms** ○ · 본문 **222자**(빈 화면 아님) ○.

### R7.5 추가 1행 — soft 항해 «뒤» `session-chip`·`reset-button` **실재** = ○

> 1차 픽스가 잃었던 자리다. 판정선은 **실재 여부**이고, 늦음은 블랙홀에서 `/overview` SSR 을
> 기다리는 만큼이라 **Q-50(PR-2) 몫**이다 — 결함으로 세지 않고 값만 적는다.

| 대상 | `/overview` 착지 | `session-chip` | `data-origin` | `reset-button` |
|---|---|---|---|---|
| 🔴 **세는 눈 선증명** — LIVE 셸 `:3191`(→ `:8061`) | 1,001ms | **실재** 1,001ms | `api` | **실재** 1,001ms |
| 대상 블랙홀 셸 `:3171` 1회 | 10,250ms | **실재** 10,250ms | `pending` | **실재** 10,250ms |
| 대상 블랙홀 셸 `:3171` 2회 | 10,236ms | **실재** 10,236ms | `pending` | **실재** 10,236ms |

- 🔴 **부정 판정식 앞에 세는 눈을 먼저 증명했다** — 같은 셀렉터가 LIVE 셸에서 «있는 것»을 1,001ms 에
  찾아낸다. 「없다」를 말하려면 「있으면 찾는다」가 먼저다.
- `data-origin=pending` 은 **블랙홀에서 옳은 값**이다(API 가 끝내 답하지 않으므로 세션은 pending 에 머문다) —
  LIVE 의 `api` 와 접두가 갈리는 것이 그 증명이다.
- 값 10.2s ≈ 구현 자기 실측 10,313ms ≈ BEFORE 10,259ms — **회귀 아님**.
- 이 한 행의 도구는 **리포 밖**(scratchpad)에 두었다. 이 발주의 write 는 `evidence/**` 이고,
  한 행을 위해 그물을 리포에 심지 않았다.

### R7.6 판정

| 축 | 판정 |
|---|---|
| 본 경로 3회 상한+ε | **3/3 PASS** |
| `/` 묶음 대조군 | **PASS**(50s checking → 2.2s) |
| `checking` 중 제안 0 | **PASS**(R-3 유지) |
| `t41_live_status_timeout` | **PASS** |
| chip·reset 실재 | **PASS**(값 기록 · 결함 아님) |

⇒ **②′ = PASS · D-3 종결.** 이로써 §R5 가 남긴 **T4-1 완결 차단분이 해소**된다.

### R7.7 §0 자수 (16대 · 이 절)

| # | 무엇이 거짓말했나 | 어떻게 드러났나 | 교정 |
|---|---|---|---|
| 1 | chip·reset 탐침을 scratchpad 에 두고 `node` 로 돌렸다 | `ERR_MODULE_NOT_FOUND: @playwright/test` — **ESM 해석은 cwd 가 아니라 «파일 위치»를 따른다**. 하마터면 「셸이 이상하다」로 읽힐 빨강이었다 | `createRequire(<tests/web>)` 로 설치된 자리를 명시 |
