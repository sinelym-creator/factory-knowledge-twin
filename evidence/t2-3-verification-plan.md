# T2-3 독립 검증 계획 v0.1 — LangGraph 조사 워크플로우
...
## 2. 🔴 착수 전 판정 회부 2건 — 이것이 정해져야 내가 «무엇을 red 로 셀지» 정해진다

### 회부① `evidenceRef.kind` 어휘가 두 정본에서 갈린다
...
### 회부② 축④ replay 강등의 «양쪽» 을 내 스택에서 못 잰다
...
|---|---|---|
| `tests/api/event_schema_drill.py` | 스키마 정본으로 이벤트 전수 검증 + `seq` 단조 + WS↔`/events` 일치 | **없다** — 스키마 정본 + HTTP만. 지금 골격 작성 가능 |
| `tests/api/credential_leak_drill.py` | 응답·이벤트·로그 전수 키·경로 스캔 + 반사 배제 + 자기 검증 | **없다** — 스캐너는 지금 작성 가능 |
| `tests/api/run_surface_drill.py` | runs 5 + graph/paths 형상·대조군 | 라우트 해제 후 실행(골격은 지금) |
| `tests/api/ssot_write_drill.py` | run 전후 SSOT 무변 실측 | **없다** — psql 스냅샷 대조 |
