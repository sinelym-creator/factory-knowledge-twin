...
| 2 tour e2e 7본 회귀 | **PASS — 회귀 0**(7/7 착지) |
...
## 2-3. tour e2e 7본 회귀 — 회귀 0 (E1)

7본 전부 돌렸다. 빨강이 선 두 본은 **前에서 같은 본을 돌려 주어를 물었다**.

| 본 | 後(`:8195`) rc | 前(`:8194`) rc | 판정 |
|---|---|---|---|
| `d71_tour_reopen` | 1 (PASS 34 / FAIL 6) | **1 (PASS 34 / FAIL 6 · 실패 줄 동일)** | **회귀 아님** |
| `t714_tour_target_cover` | 0 | - | PASS |
| `t72b_tour_repeat_control` | 0 | - | PASS |
| `t76_e11_tour_walk` | 0 | - | PASS |
| `t7b_tour_a11y` | 0 | - | PASS |
| `t7b_tour_overlap` | **2** (`step 1(headline) 에 도달 못 함`) | **2 (같은 사유)** | **측정 불가 · 회귀 아님** |
| `t7c_tour_drawer` | 0 | - | PASS |

- `d71` 의 6빨강은 그물 자신의 교정 칸(⑮ 「실패 회차 0/6」)과 새로고침 뒤 초대·콜아웃 0(⑯)이다. **前後가 글자 그대로 같으므로 이 PR 이 만든 것이 아니다** — 별건으로 회부한다.
- `t7b_tour_overlap` 의 rc **2** 는 그 러너의 「무대 미도달」 신호다. **빨강이 아니라 「못 쟀다」**이고, 前에서도 같은 자리에서 같은 사유로 멈춘다.
- 🔴 **자수 7**: `t7c_tour_drawer` 와 `t7b_tour_overlap` 을 처음엔 `--out` 없이 불러 **rc 2** 를 받았다. 그것은 대상의 답이 아니라 **내 인자 누락**이었다. 인자를 채우자 `t7c` 는 **rc 0** 이 됐다.

⇒ **회귀 0**. 이 PR 이 기존 tour e2e 를 깨지 않았다.
...
- **축 2 tour e2e 7본 회귀** — **완주 · 회귀 0**. 본문 2-3 절 참조.
