...
## §add 회부 1 판정 반영 — **배타로 좁힘**(오케 판정 09-03 14:25)

- 스키마 `allOf` **2행 추가** — `kind=preliminary` 면 `sentence` 자리에 어떤 값도 못 오고,
  `kind=sentence` 면 `preliminary` 가 못 온다. 이 검증기는 `not` 을 지원하지 않으므로
  **빈 `enum`**(`"enum": []`)으로 「이 자리에 오면 안 된다」를 표현했다 — 값이 있으면 무조건
  enum 불일치, 없으면 `properties` 가 아예 안 내려간다.
- 케이스 **2건 추가**(교차 동반 2방향).
- **대조군은 이미 서 있었다**: 좁히기 «전»에 같은 두 형상을 검증기 직접 호출로 재서 **오류 0건
  (accept)** 을 실측했고(§회부 1), 좁힌 «후» 같은 형상이 **reject** 로 뒤집혔다. 두 열이 이
  2행의 효과를 가른다.
- 수치: **74/74 통과 · 실패 0 · 자기 검증 PASS · 커버리지 56/56 · exit 0**.
- 정합: #443 은 `preliminary` / `sentence` 중 **한쪽만** emit 한다(`workflow.py:216`·`:239`) —
  좁힌 결속과 충돌 없다.
