...
## 2026-09-04 (promotions 10-14, defect closures)

- develop promoted to main five more times on the same day: 10th `0c2bc38` (#602), 11th `daf5b8d` (#611), 12th `09151b6` (#622), 13th `b4b06dd` (#647), 14th `879fc35` (#665). Each carried operator-reported screen defects fixed the same day and re-verified by the verification seat (D-48~D-68 lineage; see PROGRESS entries of the day).
- Live gateway kept ON as a standing order (operator 18:55); a logon-triggered scheduled task (`FKT-Gateway-On`) restarts it after reboot. Contract v0.1.15/v0.1.16 appended.

## 2026-09-05 (promotions 15-21, develop/production split)

- Promotions 15th `56d5730` (#684) through 21st `d4ebe35` (#796): 16th `aa48b2d`, 17th `7fa6263`, 18th `e599dad`, 19th `4d39bde`, 20th `980393a`. External public-surface rechecks landed for each (`evidence/promo15/` onward).
- T7-42: develop stage separated from production (`infra/develop-stage.ps1`, `:8020`/`:8797` vs `:8010`/`:8787`); a five-step promotion procedure (develop merge → develop-stage verification → request → operator approval → production) written into CLAUDE.md after an unapproved production-path change was self-reported.
- T7-43 README rewritten for a general reader; T5-1 search/answer axis completed; D-84 (safety-rule omission in live synthesis) detected, fixed and re-verified on the develop stage.

## 2026-09-06 (promotions 22-25, Gate 7, CI)

- Promotions 22nd `5177e74` (#811, first executed under delegated approval), 23rd `2cd75f4` (#834 — executed without the required main-deploy approval; self-reported 09:00, rule re-stated: main deploys always need operator approval), 24th `5f17b54` (#862), 25th `99d3cab` (#895).
- T5-3 CI gates completed (ledger 67/68): seed-db, benchmark-smoke, unit jobs for ai-api and web-console, container scan, license inventory, public endpoint policy. O-48 evidence-flag path landed (#872~#886). D-88 production `document_chunk` index rebuilt after a live vector failure (operator-approved recovery, D-16 shape).
- Gate 7 judged (PASS 11 · conditional 1 · partial 2 · FAIL 0); the two partial items remained open until 09-11.

## 2026-09-11 (promotions 26-32, ledger 68/68)

- Operator device-test defects E-1~E-7 (tour steps, reset/home, approve stall, badge staleness) fixed, independently verified and promoted the same day: 26th `30274ba` (#909), 27th `e02d08e` (#922), 28th `81aec01` (#949), 29th `4225b4f` (#971), 30th `ceebc34` (#976), 31st `509f625` (#991), 32nd `c35377d` (#1006). Public-surface external recheck PASS recorded for each (`evidence/promo26..32-external-recheck.md`).
- Live call caps per operator directive: 3 per session and 3 per hour (contract v0.2.4, CAP3); when the LLM path is unreachable the badge no longer claims live. Failure latch (900 s) and hourly-cap wording verified on a dedicated stage (`evidence/latch-verify.md`). D-97 (retry consumes two calls per run) measured and made visible via `synthesis.calls` (contract v0.2.5).
- T5-1 closed with the final evaluation report (`benchmarks/eval-report-final.md`, raw re-aggregation, no LLM judge) and a README "benchmark · latency" section. T5-2 closed: Gate 7 partial items settled by SQL-layer static audit (`#1003`) and structural immunity for WebSocket input (`evidence/t5-2-gate7-close.md`). **Ticket ledger 68/68, open defects 0.**
- D-92 (a cleanup order deleted develop-stage DB volumes after matching containers by name prefix) recorded; container ownership is now judged by network/DSN references, never by prefix. D-98 (compose bootstrap failing on an empty boolean env) fixed (#998).
- Seat configuration normalized (12/12 dedicated settings), remote lanes pruned to develop/main plus one held PR (#617).

## 2026-09-22 (incident D-99: production Docker outage)

- D-99: the production Docker Desktop instance was stopped by an external TERM on 09-20 at 20:09:37 (Electron tree exit 143, the MSYS kill signature), taking the three production containers plus `:8010` and `:8787` down for 2 days 3 hours while the live surface was ON. Recovered 09-22 22:54 (containers up, `/api/health` ok on build `509f625`, external recheck PASS). No repository change was involved.
- Trigger established by cross-log comparison with the Lelouch session (E1): a `timeout 290 gh pr checks --watch` wait expired in the same second as the stop (1 occurrence in 44 uses). Which pid received the signal was not measured on either side, so the incident is closed as "trigger highly likely, mechanism unproven"; a reproduction experiment is held as separate work. The credential-rotation axis was ruled out (E1).
- Prevention: waits that wrap native process trees (`timeout N <cmd>` and the like) are abandoned fleet-wide in favour of single checks or Monitor, with `--foreground` required where unavoidable; targeted `taskkill /PID <launcher> /T` is kept, since a named-target kill differs in shape from an expiring group signal. On this host a 15-minute scheduled task `FKT-Docker-Health-Watch` now starts the Docker daemon, the three production containers and the two ports when it finds them down — it never stops anything — so recovery no longer depends on a session being awake.
