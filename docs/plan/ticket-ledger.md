...
| Q-5 | spec §4 pgvector 보유분 2건(`MaintenanceRecord.note`·`FailureMode.description` 임베딩) 미착지 — 명시 제외 vs 착지 «판정 이연»(T2 검색 전략 구현 진입 시 필요 실측으로 오케 판정 · 이연 자체를 여기 성문 — 조용한 누락 아님) | levi2 4대 부수 계수(vector 칼럼 전수 = document_chunk.embedding 1개) · 🔴 T2-1 AC에 실측 소견 결속(08-30) | T2-1 완료 보고 시 오케 판정 |
...
| Q-18 | 잔여 501 라우트 티켓 귀속 미정 — `POST /sessions`(저장소)·`GET /scenarios`(질문 정본 표면)·`GET /graph/paths` = T2 분해안에 명시 귀속 없음. T2-1 잠정: sessionId 형식 검증만·allowlist 서비스 내 읽기전용 | senku2 9대 T2-1 게이트 1 실독(08-30) | T2-2 발주 시 오케 귀속 확정 |
