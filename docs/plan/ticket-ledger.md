...
| Q-39 | 딥링크 방문자가 «조용히» 세션을 얻는다 — 셸 `<Link>` 프리페치가 /overview→/ 연쇄로 Set-Cookie 유발(실측: 2.5초 후 쿠키 2개 · /evidence·/documents 응답 자체는 Set-Cookie 없음 = AC 참 유지). 🔴 처방 방향 성문 = 「프리페치는 «입장»이 아니다」 — `_rsc`·프리페치 표지 요청에 세션 미발급(apps/** 2~3줄 · T3-2 픽스 lane과 분리 — 재검 판정선 보호) | senku2 14대 자수(08-30 · E1 — 「대기 없이 센 것」 자기 진단) · 오케 판정 ⓐ | 재부팅 후 소조각 |
