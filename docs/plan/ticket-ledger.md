...
| T3-4 | 조사 실행·전략 비교 화면 — run 타임라인(이벤트 스트림) · replay 재생 UI(Q-27 화면 축 판정) · graph evidence 시각화(`/graph/paths?from&to` 소비처 — Q-26 재판정) | 구현 | wireframes ⑤ + deterministic replay 소비 | T3-1 | 🔴 **최소 형상 발주 08-31 10:15**(Feature Freeze 조기 — 폐하 재가 09:57 ⓑ · 센쿠2 15대 · `lane/senku2-t3-4-run-screen` @ 4791a8c · 축소 범위 = 티켓 T3-4.md 「최소 형상」 표 · 이연 = 단계 필터·배속·스크러빙·자유 입력·그래프 도형 = 배포 후 트랙) |
...
| Q-37 | Next rewrite 목적지가 «빌드 시점»에 routes-manifest로 구워진다 — `FKT_API_BASE`를 start에만 주면 무효(기본값 8000이 박혀 «남의 서버»를 잰다 — T3-3 측정 함정 실증 · 대조군으로 오진 차단). 「포트 바뀌어도 화면 코드 불변」 주석은 배포 축에서 반만 참 — 목적지 변경 = 재빌드 | senku2 14대 T3-3 실측(08-30 · E1) · 오케 판정 | Phase 4(Tunnel·배포) 결속 — env 주입 시점 설계(빌드 타임 vs 런타임 프록시) |
...
| Q-42 | HTTP 표면 드릴 «전부»의 귀속 부채 — 「FKT_API_BASE가 가리키는 서버가 내 트리를 읽는가」를 증명하는 단이 `ssot_write_drill` 하나뿐(나머지는 주장 `FKT_SERVER_REPO`만). Q-40 `prove_colocation` 패턴을 드릴 공통 전처리로 일반화(미증명 = exit 2) | levi2 12대 Q-40 회부 ②(08-31 · E1) · 오케 등재 09:58(확대는 별건 발주 — 좌석이 넓히지 않음 = 옳은 처리) | 검증 좌석 대기열 — 순서 Q-41 → Q-42(D-3 재검 PASS 10:14로 선행 해소) |
