...
## 원장 (진행률 = ✅ 32 / 총 37 — 🔴 **T3-1 완결 08-30 18:07**(독립 검증 PASS · PR#160 착지 4cc02e7) · 분모 31→37 선언(Phase 3 6티켓 등재 08-30 16:05 · 진입 재가 15:58 「승인」 + Phase 2 게이트 통과 동시) · **Phase 2 완결: T2-6 완결 08-30 15:53**(통합 검증 PASS · PR#145 · 오케 통합 판정) · T2-5 완결 08-30 15:18(독립 검증 PASS · PR#137 착지 1146ca4) · T2-4 완결 08-30 09:17(재검 PASS · PR#129) · T2-3 완결 04:30(PR#121) · T2-2 완결 03:17(PR#114) · T2-1 완결 01:41(PR#108) · Phase 2 진입 재가 00:04 · Phase 1 완결 08-29 19:05 · 분모 이력: 🔴 25→31 T2-1~T2-6 등재 · 24→25 T1-10 등재 · 15→24 Phase 1 분해)
...
| T3-3 | Evidence·Documents 화면 — 근거 열람·STALE 배지 표면(Q-20 소비처)·딥링크(Q-16 예외 축 소비) | 구현 | wireframes ③ | T3-1 ✅ | 🔶 구현 착지(PR#167) · 🔴 1차 검증 «불합격»(리바이2 11대 PR#172 · 판정문 evidence/t3-3-evidence-screens-verification.md — D-3 = Q-39 실증: 프리페치 세션이 진짜 입장 = 「열람만」 위반. 초록 5축 유지: 인용 항등식 화면까지·🔴 STALE 주입 실증 = Q-20 울음 판정선 통과·Q-34 성문 준수·망축·무접촉) · 픽스 진행(lane/senku2-t3-3-fix) → D-3 축 단축 재검 예정 |
...
| Q-39 | 딥링크 방문자가 «조용히» 세션을 얻는다 — 셸 `<Link>` 프리페치가 /overview→/ 연쇄로 Set-Cookie 유발(실측: 2.5초 후 쿠키 2개 · /evidence·/documents 응답 자체는 Set-Cookie 없음 = AC 참 유지). 🔴 처방 방향 성문 = 「프리페치는 «입장»이 아니다」 — `_rsc`·프리페치 표지 요청에 세션 미발급(apps/** 2~3줄 · T3-2 픽스 lane과 분리 — 재검 판정선 보호) | senku2 14대 자수(08-30 · E1) · 🔴 판정 «변경»(08-31 00:00 — 이연 ⓐ → 즉시 픽스): 리바이2 T3-3 검증이 전제를 뒤집음 — 프리페치 발급 세션이 **ai-api에서 실제로 선다**(진짜 입장 · v0.1.6 「열람만」 정면 위반 = T3-3 D-3 FAIL 원인). 「생겼다」에서 멈춘 이연 판정보다 「서는가」까지 물은 검증이 옳았다(오케 귀속) | 픽스 = lane/senku2-t3-3-fix(프리페치 표지 미발급 — 표지 실측 선행) |
| Q-40 | `replay_fixture_drill` F-11 실패 — fixture «부재» 시나리오가 501 기대인데 **200**(워크트리·메인 트리 양쪽 재현 · T3-3 서버 변경 0이라 귀속 별건). 원인 특정 필요: 데이터 상태(어느 실행이 fixture를 만들었나) vs 회귀 | levi2 11대 T3-3 검증 R-6(08-30 · E1) | 🔴 재부팅 후 1착 대기열(검증 좌석 triage) |
| Q-41 | `mode-badge` 전이 E2E 간헐 실패(단독 통과 · 전체 실행 실패 — 타이밍 의존 의심) | levi2 11대 T3-3 검증 R-7(08-30 · E1) | 재부팅 후 대기열(검증 그물 안정화 축) |
