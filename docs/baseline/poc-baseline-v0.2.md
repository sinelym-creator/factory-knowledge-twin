...
- 🔴 **D-21 ⓒ(09-02 18:13 · 폐하 결정 2 ⓐ)**: 공개 셸(Vercel 경로)의 조사 «진행 표시»는 WS 실시간이 아니라 **주기 조회 대체**(계약 v0.1.10 · `GET /runs/{runId}/events` · seq 중복 제거 · terminal 종료)로 성립한다 · 직결 경로는 WS 실시간 유지 · §21 「WebSocket streaming」 항의 공개 셸 해석 = 이 줄이 정본 · WS 미개통(D-21)은 해소가 아니라 대체다.
- 🔴 **T6-1(09-02 · 폐하 결정 3)**: Live 진단 synthesis = 운영자 PC 로컬 Claude Code CLI(구독 · API 키 0) 경유 게이트웨이(§15.2 소유자 통제 시연 + fixture 녹화 · 공개 API 0 · 컨테이너 자격 증명 0) · 근거 결속 위반 = 전량 거부(`axis=live-rejected` 드러냄 · 조용한 폴백 0) · **§33 「Diagnosis synthesis 10초」 잠정 목표 = 실측 미달**(Actual 합성 step 18.4~19.1s(벽시계 run 21.5~26.7s) 벽시계 · CLI 내부 센쿠2 CLI 내부 10.9~13.6s · n=3(리바이2)+n=5(센쿠2) · 독립 검증 #410) → 수치 개정 여부 = 폐하 결정 회부(이 줄은 회부 기록 · 수치 유지).
- 🔴 **§15.2 개정(09-02 22:27 · 폐하 결정 4 · §0.3)**: 「공개 사용자의 요청은 Replay 또는 로컬 모델로 처리」 → **「소유자가 소유자 PC 의 합성 게이트웨이(구독 Claude Code CLI)를 켜 둔 동안만 공개 방문자의 조사도 실제 LLM 합성으로 처리한다(PoC 한정 · 제3자 판매 0 · 방문자 세션당 조사 상한 · 구독 한도 도달 시 녹화 재생으로 강등하고 화면에 드러낸다) · 꺼 두면 녹화 재생」**. 근거 = 폐하 22:12~22:27 DM(「PoC·구독 요금제·내 PC 서버·프런트만 Vercel = 정책상 문제 없음 · 옵션 ON 일 때만 LLM」). 「Claude credential·session·token 을 Browser·public API 에 전달하지 않는다」는 유지(게이트웨이 토큰은 컨테이너↔호스트 사이에만). 집행 = T6-2.
