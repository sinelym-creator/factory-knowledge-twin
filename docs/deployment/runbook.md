...
| **Funnel «설정» 복원** | 재부팅 «후» `tailscale funnel status` | **복원됨** — 2026-09-11 09:11:47 재부팅(`LastBootUpTime`) 뒤 손대지 않은 상태에서 00:03 `funnel status` = `:8443` on · 게이트웨이 `:8787`·컨테이너 3본 생존 | E1 (1회) |
