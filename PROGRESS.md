...
| Phase | 티켓 | 담당 |
|---|---|---|
| 6 | **§35.7 최종 관문(D-004) 폐하 판정 대기** — 상신 09-02 15:59(팀 채널 · 「Release 후보 — 축소 적용(v0.3)」 · 충족 3/부분 3/미충족 4 · 원장 42/47 · 상한 D-2) + **D-21 결정 청구**(ⓐ 권고 · 층 판정 = **Vercel rewrite 쪽** E1 16:03 리바이2 #388: 같은 쿠키·같은 run 으로 Vercel 경유 opened=false / Funnel 직결 opened=true · 사정거리 = 직결은 tailnet self) · 견적 ⓑⓒ = 센쿠2 보고 대기 · 승인 착신 시 = `docs/decisions/004` 승격 → T5-1~T5-5 축소 적용 판정 → CHANGELOG 오늘분 | 오케(상신·승격) + 리바이2(Q-73 다음) + 센쿠2(견적) |
...
| Phase | 태스크 | 선결 |
|---|---|---|
| 6 | **최종 관문 = D-004 상신 15:5x**(§35.7 · 「Release 후보 — 축소 적용(v0.3)」 · 충족 3/부분 3/미충족 4) + **D-21 결정**(ⓐ 권고) → 승인 시 `docs/decisions/004` 승격 · 원장 T5-1~T5-5 축소 적용 판정 · CHANGELOG 오늘분 · 남은 소조각 = Q-73(networkidle 2칸 · 검증) · OFF 창 재현 1회 · 외부 vantage 워커 상한(4·2·1) · `API_BASE` 공용 상수(tests/api) · `t44_outage_watch` 자식 rc 판정 · Gate 7 ⑦⑧⑪ 대조군 서버 2본(릴리스 뒤) | T3-6 ✅ #386 15:46 · 발췌 대조 CI ✅ #387 15:40 · **진행 중 15:50~** = 리바이2 D-21 층 분리 + D-20 스캐너 1줄 · 센쿠2 D-21 ⓑⓒ 견적(코드 0) |
