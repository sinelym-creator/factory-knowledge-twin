...
| Phase | 티켓 | 담당 |
|---|---|---|
| 6 | 🔴 **T6-4·T6-5 미측 7건 판정**(완료 주장의 마지막 칸 · 추정 초록 0) — P0 ①투어 중간 재개 ②키보드 완주 ③투어 문면 공개 경계 / P1 ④reduced-motion ⑤390 바텀시트 / P2 ⑥「후」 50컷 시각 검수 ⑦O-1 귀속(overview testid 46/69) · lane `lane/levi2-t65-untested` · 대상 `d15b534` | 리바이2 34대 · 발주 09-03 16:23 |
| 6 | 🔴 **배포 축 「실제 HTTP 401」 측정** — 전임이 위험(공개 방문자의 가짜 합성 노출)으로 못 잰 1건 · 안전 열쇠 = 게이트웨이를 **토큰 불일치**로 세워 모든 요청을 401→replay 로만 끝냄 · 🔴 좌석 코드 검산 통과 시에만 집행(거짓·미판별이면 미측 유지) · 창 ≤5분 | 센쿠2 35대 · 발주 09-03 16:24 |
| 6 | 🔶 **잔여 정리**(비차단) — 🔴 정지 컨테이너 2본이 남의 워크트리를 문다(컨테이너 먼저 · D-13 계보) · 회귀 후보 3본 · 보존 `-ad788d3` 는 오늘 유지(롤백 경로) · named volume 오늘 보존 | 센쿠2 35대 |
