...
| Phase | 티켓 | 담당 |
|---|---|---|
| 6 | 🔴 **T6-2 소유자 스위치 live 합성(P0 · 폐하 결정 4 22:27 · 원장 48→49)** — 배포 스택 ↔ 소유자 PC 게이트웨이 연결(bind·토큰) + 방문자 상한(3/h → 429→replay) + README 「공급자 교체 가능한 합성 축」 · 발주 22:38(센쿠2 31대) · 검증 = 리바이2 28대(ON/OFF · 외부) · **진행 23:28 = wip `3c86c7d`(⑥ sonnet/medium · UTF-8 · switch.ps1) · ②④⑤ 미착수 · ③ = 이미지 교체(배포 이미지 `6e1487d` = T6-1 이전 · 센쿠2 발견 23:11) · 드릴 무효 0/4(합성 단계 미도달 · 재부팅 후 1착 재측)** · 승격 4회차 | 센쿠2 + 오케 + 리바이2 |
...
| ✅ **D-22 reset-modal 귀속** | 대상 결함 아님 · 진범 = 그물 전제(`FallbackBanner` 는 online 이면 안 뜸) · 손잡이 1개 프록시 3열(자극 계수 overridden 0/9) · 그물 수정 PR 후보(`reset-modal.spec.ts:72` → `getByText(...).toHaveCount(0)`) | 원장 D-22 · #414 |
