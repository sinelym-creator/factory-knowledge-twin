...
| Phase | 티켓 | 담당 |
|---|---|---|
| 6 | 🔴 **T6-1 Live 진단 synthesis(P0 · 폐하 결정 3 19:14~19:17 「LLM 합성 = 완성 조건 · 구독 CLI · API 미사용」 · 원장 47→48)** — 발주 19:20(센쿠2 31대 · 게이트웨이 + `live_synthesis` + 이벤트·화면 + 기록본 replay) · 계약 v0.1.11 + 스키마 additive(오케) · 검증 = 리바이2(D-21 ⓒ 판정문 뒤 발주) · 상한 오늘 밤 → main 승격 | 센쿠2 + 오케 + 리바이2 |
| 6 | **D-004 집행 → 릴리스 뒤 개선 1사이클(폐하 「전건 권장안 승인」 18:13 · 스자쿠 22대)** — ✅ main 승격 #395(공개면 = README 제약·decisions/004 · 외부 재검 PASS) · ✅ verdict 관문 행 #394 · ✅ 계약 v0.1.10 #396 · ✅ D-21 ⓒ 구현 #397(429 = 드러냄+중단) · ✅ Q-73 그물 개정 #398(회귀 미실행) · 진행 = 승격 #399 → 리바이2 외부 검증(폴링) + 회귀 계수(CPU 창 ~19:45 뒤 go) → SSOT 묶음 2 | 오케 + 센쿠2(#397) + 리바이2(#394·#398·검증) |
...
| Phase | 태스크 | 선결 |
|---|---|---|
| 6 | T6-1 독립 검증 발주(리바이2 · 근거 결속·갈림·egress·fixture 심사·GS-01) · 승격 3회차 · SSOT 묶음 2 · README 「AI 조사」 문면 = LLM 착지 뒤 정렬 | T6-1 PR 착지 |
...
| 🔶 **D-21 ⓒ 폴링 전환(1순위)** | ✅ 구현 착지 #397(계약 v0.1.10 #396 · 셸 1파일 · 새 서버 코드 0) · 429 = 드러냄+조회 중단(자동 재개 0) · 잔여 = Production 외부 검증(리바이2 · 창 뒤 go) + 그물 개정(1006 문면 대기 자리) · 🔴 WS 미개통 자체는 남음(대체 ≠ 해소) | D-004 · 원장 D-21 · #396 · #397 |
...
| 🔶 **Q-73 networkidle** | ✅ 그물 개정 착지 #398(코드 45곳 3분류 = 제거 27·앵커 17·유지 1 · 131/14 동수) · 잔여 = 회귀 계수 §7(창 뒤 go · 별건 PR) → 완료 판정 | 원장 Q-73 · #398 · `evidence/q73-nets-revision.md` |
