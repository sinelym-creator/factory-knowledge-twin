 ...
 */
/**
 * 🔴 **기본값을 두지 않는다**(D-76 · D-72·D-74 와 같은 계보).
 *
 * 앞판은 `?? "http://127.0.0.1:8000"` 이었다. 그 값은 이 리포에서 **센쿠2 좌석의 대역**이고,
 * 지정 없이 구운 셸은 그 자리를 «가리킨 채» 굳는다 — 빌드는 성공하고, 화면은 뜨고, 남의
 * 무대에 대고 말한다. 실패하지 않는 오지정이라 아무도 알아채지 못한다.
 *
 * 🔴 **죽는 자리는 여기 하나다.** 아래 `env` 가 이 값을 `FKT_API_BASE_BUILD` 로 구워 런타임에
 *    넘기므로, 빌드가 섰다면 그 상수는 반드시 있다. 두 자리에서 각각 「미지정」을 판정하면
 *    같은 사건에 문면이 둘 생긴다.
 */
const API_BASE = process.env.FKT_API_BASE;
if (!API_BASE) {
  throw new Error(
    "[FKT] 빌드 중단 — FKT_API_BASE 를 지정하라(기본값 없음 · 무접촉 대역을 굽지 않는다)",
  );
}
