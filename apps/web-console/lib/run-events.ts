 ...
 * 🔴 **1006 은 사유를 가리지 못한다** — 공개 셸에는 WS 라우트가 없어 핸드셰이크가 404 로 끝나고
 *    (세션은 그대로 살아 있다), 무쿠키·깨진 세션은 403 으로 거부되며, 붙은 뒤의 절단도 같은 1006 이다.
 *    셋이 한 코드로 오므로 문구는 원인을 «단정하지 않고» 지금 상태와 대체 경로만 말한다(Q-69).
 */
