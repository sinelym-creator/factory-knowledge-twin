...
function TourStepView({
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      /* 🔴 **D-89 — `settled` 를 «바깥 가드»에 두면 그것이 래치가 된다.** 아래 주석이
         약속한 것은 「도달했다≠계속 도달해 있다 … 래치를 걸지 않고 창이 닫힐 때까지 다시 본다」인데,
         이 자리에 `!settled` 가 있으면 한 번 참이 된 뒤로는 **다시 계산조차 되지 않는다** — 문면과
         거동이 갈린 자리였다(M-2 실측: 412×600 direct 에서 문서가 100ms 에 52px 자라고, 성장 «전»에
         든 회차는 링이 `bottom 615 > vh 600` 으로 밀린 채 굳었다 · 前 재현 4/8).
         🔴 비파괴 판별로 「계기 부족」이 아님을 갈랐다: 정착 뒤 프로그램 스크롤 1px 로
            `measure` 를 다시 돌렸는데도 보정이 0 이었다(2/2) — 막은 것은 이 가드다.
         🔴 멈춤 보장은 그대로다. 안쪽이 매번 다시 계산해 **제자리면 `return`** 하고,
            사람이 움직이면 `userMoved` 로 즉시 손을 떼고, 폴링은 8초에 끝나며 스크롤은 450ms 간격이다. */
      if (!userMoved) {
