...
- 🔴 **공개 API 로 구독이 나가지 않습니다.** 게이트웨이 주소 env 가 없으면 그 축의 코드는 **적재조차 되지 않고**, 그래서 공개 Sandbox 의 `online:false` 는 결함이 아니라 **참**입니다.
...
- **상한**: 세션 하나가 쓸 수 있는 Live 조사 횟수는 기본 **3회/시간**입니다(`FKT_RUN_CAP_PER_SESSION`). 넘기면 `429` 로 거절하되 **녹화 재생은 그대로 열려 있습니다** — 재생은 구독을 쓰지 않습니다. 운영 절차는 [runbook §7](docs/deployment/runbook.md) 을 보세요.
