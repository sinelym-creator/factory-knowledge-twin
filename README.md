...
3. **AI 조사 시작** — 에이전트가 조사 계획을 세우고 DB 조회 → 문서 검색 → 관계 추적을 **단계별로** 진행합니다. 과정이 전부 화면에 흐릅니다 — 블랙박스가 아닙니다. 진행 표시는 **직결 경로에서 실시간 스트림(WebSocket)**, **공개 셸에서는 주기 조회**입니다(아래 「알려진 제약」).
...
- 🔴 **화면은 숨기지 않습니다.** 스트림이 안 열리면 「실시간 스트림 대신 주기 조회로 진행
  중입니다 — 2초마다 서버에 다시 묻습니다」를 그대로 띄웁니다. 조용히 메우고 실시간인 척하지
  않습니다 — 잃는 것은 «즉시성»이므로, 화면이 그 사실을 말합니다.
...
- **폴링 전환은 착지했습니다**(D-21 ⓒ · 릴리스 뒤 개선 1순위였던 항목). 위 되감기 배관을
  ...
  경우(101 전 close · 1006)에만 `GET /runs/{runId}/events` 를 2초 간격으로 조회합니다.
