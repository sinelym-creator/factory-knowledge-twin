...
LLM 합성은 켜고 끌 수 있는 부가 기능입니다. 소유자 PC의 Claude Code CLI(구독)를 감싼 로컬 게이트웨이가 떠 있을 때만 붙고, 응답이 조사 근거 밖을 인용하면 그 응답은 통째로 버립니다. 구독이 공개 API로 나가는 일은 없습니다. 세션당 Live 조사는 시간당 5회까지이고, 재생은 횟수 제한이 없습니다.
...
**성능 수치는 아직 없습니다.** 지연(P50/P95)과 벤치마크는 측정 전이라 빈 칸으로 둡니다. Live 합성은 2026-09-05 production 재검에서 1건 15.6초, develop에서 3건 12~14초가 나왔고, 잠정 목표 10초에는 못 미칩니다.
