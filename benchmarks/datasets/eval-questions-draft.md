...
| E4 가설 | ◇ 항목(T0-6 **미명시** — Phase 1 데이터 생성 요구) 中 **아직 충족되지 않은 것**. 🔴 2026-08-29 기준 §5 존치 요구 5건(D-2·D-3·D-5·D-6·D-8)은 **전건 E1 충족**으로 올라갔다 — 이 등급에 남은 것은 `Q-SAFETY-001`의 **자리표시자 앵커**(§6 U-7)뿐이다 |
...
> 🔴 **다만 위 앵커 표기는 아직 자리표시자다** — `DOC-SAF-xxxx`·`DOC-SAF-yyyy`·`SAF-PPE-nn`. 실값은 DB에 있다(E1 · 2026-08-29 실측): `SAF-LOTO-01` → `DOC-SAF-0029@r3` · `SAF-PPE-01` → `DOC-SAF-0030@r3`. **앵커 확정은 문항 본문 변경이므로 오케 판정 영역**이다 — §6 U-7로 올려 둔다. 데이터 요구(D-6)의 충족과 앵커 표기의 미확정은 **다른 것**이다.
...
| U-7 | 🔴 `Q-SAFETY-001` 앵커 자리표시자 확정 — `DOC-SAF-xxxx`·`DOC-SAF-yyyy`·`SAF-PPE-nn`. **실값은 이미 실측했다**(E1 · 2026-08-29): `SAF-LOTO-01`→`DOC-SAF-0029@r3` · `SAF-PPE-01`→`DOC-SAF-0030@r3`. 문항 본문 변경이라 좌석이 임의로 치환하지 않는다 | 즉시 가능 | **오케 판정** |
...
1. ~~**◇ 5건이 여전히 E4(가설)** — Phase 1 데이터 생성이 §5 존치 요구를 충족하지 못하면 `Q-DIRECT-002`·`Q-DIRECT-003`·`Q-MULTIHOP-002`·`Q-MULTIHOP-003`·`Q-SAFETY-001` 5문이 측정 불가가 된다. v0.1의 ◇ 10건에서 **절반으로 줄었으나 0은 아니다.**~~
   ...
   - 🔴 **그래도 「0」이라고 쓰지 않는다.** 남은 것은 성질이 다르다: `Q-SAFETY-001`의 앵커가 아직 자리표시자(`DOC-SAF-xxxx`·`DOC-SAF-yyyy`·`SAF-PPE-nn`)다. **데이터는 있는데 표기가 없는 상태**이며 확정은 오케 판정이다(§6 U-7). 「데이터 요구 미충족」과 「앵커 미확정」을 한 칸에 섞으면 다음 사람이 또 stale을 읽는다.
