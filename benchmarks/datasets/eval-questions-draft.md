...
| E4 가설 | ◇ 항목(T0-6 **미명시** — Phase 1 데이터 생성 요구) 中 **아직 충족되지 않은 것**. 🔴 2026-08-29 기준 **이 등급은 비었다** — §5 존치 요구 5건(D-2·D-3·D-5·D-6·D-8) 전건 E1 충족 + `Q-SAFETY-001` 자리표시자 앵커도 실값 확정(U-7 닫음). 🔴 **「비었다」를 「전부 측정됐다」로 읽지 마라** — chunk 입도 **바인딩**은 닫혔지만(U-1 · §8), 그것은 «좌표»의 실측이지 «검색 성능»의 실측이 아니다. §3.1·§3.2의 `Actual`은 전량 `Not measured` 그대로다 |
...
> **◇ 의존 — 충족(E1 · T1-2 실측 · 2026-08-29 재검)**: SOP 본문의 절 구조는 T0-6 미명시(§5 D-3)이나 Phase 1 데이터가 채웠다 — SOP revision 전건에 `## 4.`·`## 5.` 절 실재. 상설 그물 = `tests/data/seed-integrity.sql` **C-5**.
...
> **◇ 의존 — 충족(E1 · T1-2 실측 · 2026-08-29 재검)**: r1↔r2 값이 **실제로 달라야** 한다(§5 D-2) — `content_sha256` 2종·90분↔120분·공구 4↔5로 충족. 상설 그물 = `tests/data/seed-integrity.sql` **C-3**(본문 상이)·**C-4**(지금 인용 가능 revision 1건).
...
> **◇ 의존 — 충족(E1 · T1-2 DB 실측 · F-1 수정 후 2026-08-29 재검)**: `MR-2025-0087`(2025-02-11 · `EQ-CNC-204` · replace)이
...
>
> ◻ **채점자 주의(E1 · F-1 수정 후 재실측 2026-08-29)**: `EQ-CNC-204`의 정비 이력은 전량 4건이고
> 그중 **고장 모드가 매핑된 것은 `MR-2025-0087` 1건뿐**이다 — 「유사한 이력」의 정답은 1건이다.
>
> | ID | 수행일(KST) | 유형 | 상위 WorkOrder | 고장모드 |
> |---|---|---|---|---|
> | `MR-2026-0006` | 2026-06-02 | lubricate | — | — |
> | `MR-2025-0095` | 2025-11-09 | inspect | — | — |
> | **`MR-2025-0087`** | **2025-02-11** | replace | `WO-2025-0087` | `FM-BRG-WEAR` |
> | `MR-2024-0004` | 2024-04-19 | calibrate | — | — |
>
> ✅ **ID 연도 = 실제 수행 연도가 회복됐다**(F-1 수정 · 40건 전건 정합 · 재검 20/20). 이전 개정이 경고했던
> 「ID 연도로 시점을 읽으면 오독한다」는 **해소**됐다 — 다만 채점 기준은 그대로 `performed_at`을 쓴다.
> ID는 사람이 읽는 표지이고 시점의 정본은 값이다.
...
> **◇ 의존 — 충족(E1 · T1-2 실측 · 2026-08-29 재검)**: `FM-TOOL-IMB`에 SOP 미매핑 상태 보장(§5 D-5) — 전역 미매핑 1건 + `EQ-CNC-204` R09 직결로 충족. 상설 그물 = `tests/data/seed-integrity.sql` **C-6·C-7**.
...
```yaml
expected_asset_ids:      [EQ-CNC-204 ◆, CP-204-BRG-01 ◆]
relevant_document_ids:   [DOC-SOP-0014 ◆, DOC-SAF-0029 ◇ (SAF-LOTO-01 정의 문서),
                          DOC-SAF-0030 ◇ (PPE 규정 문서)]
relevant_revisions:      [DOC-SOP-0014@r2 ◆, DOC-SAF-0029@r3 ◇, DOC-SAF-0030@r3 ◇]
expected_graph_path:     [CP-204-BRG-01 ◆, FM-BRG-WEAR ◆, SOP-BRG-INSP-014 ◆,
                          SAF-LOTO-01 ◆, SAF-PPE-01 ◇]
relations:               [R08 ◆, R11 ◆, R12 ◆, R22 DOCUMENTED_BY ◆]
required_safety_rules:   [SAF-LOTO-01 ◆, SAF-PPE-01 ◇]
...
> **◇ 의존 — 충족(E1 · T1-2 실측 · 2026-08-29 재검)**: PPE SafetyRule ID 확정(§5 D-6) — `SAF-PPE-01` 실재. 상설 그물 = `tests/data/seed-integrity.sql` **C-8**.
> 🔴 **앵커 자리표시자 확정 — 2026-08-29 집행(U-7 · 오케 승인)**. `DOC-SAF-xxxx`→`DOC-SAF-0029` · `DOC-SAF-yyyy`→`DOC-SAF-0030` · `SAF-PPE-nn`→`SAF-PPE-01` · `"각 안전 문서의 current_revision"`→`DOC-SAF-0029@r3`·`DOC-SAF-0030@r3`.
> 근거는 **두 좌석의 독립 교차 일치**다 — 검증 좌석의 파일 실측(`safety_rule.current_revision_id`)과 구현 좌석의 DB 재도출이 따로 같은 값에 닿았다. 한쪽만이었으면 집행하지 않았다.
> 🔴 **표기는 ◇로 둔다.** ◇의 뜻은 「T0-6 미명시」(§0.5)이고 그 사실은 여전히 참이다 — 값이 채워진 것과 T0-6이 확정한 것은 다르다. 실값이 생겼다고 ◆로 올리면 스펙 이력이 사라진다.
...
| D-2 | `DOC-SOP-0014` revision 2개 + **값 상이** | `Q-DIRECT-003` | ◻ **부분** — revision 2개 이상 문서 8건 보장(§5) · `@r1`/`@r2` 확정(§6) / **값 상이는 미명시** | ✅ **충족**(T1-2 · E1) — `content_sha256` 2종 · 90분↔120분 · 공구 4↔5 |
| D-3 | SOP 본문 절 구조(필요 공구·작업 시간) | `Q-DIRECT-002` | ◻ **부분** — SOP entity 속성은 확정 · **문서 본문 절 구조 미명시** | ✅ **충족**(T1-2 · E1) — SOP revision **25/25**에 `## 4.`·`## 5.` 절 |
| D-4 | 미존재 설비 ID 확보 | `Q-UNANS-002` | ✅ **PASS** — T0-6 §5가 `EQ-CNC-999`를 명시 지정 | **닫음** ✅ |
| D-5 | SOP 미매핑 FailureMode 1건 | `Q-MULTIHOP-003` | ◻ **부분** — `FM-TOOL-IMB` 실재(§6) · **SOP 미매핑 상태는 미보장** | ✅ **충족**(T1-2 · E1) — **전역** 미매핑 1건(`FM-TOOL-IMB`) · R09 직결 |
| D-6 | PPE SafetyRule ID 확정 | `Q-SAFETY-001` | ◻ **부분** — SafetyRule 8건·「LOTO·PPE 등」(§5) · **PPE ID 미확정** | ✅ **충족**(T1-2 · E1) — `SAF-PPE-01` 실재 |
...
**집계**: ✅닫음 4 (D-1은 재설계로 해소) · ✅**Phase 1 충족 5**(D-2·D-3·D-5·D-6·D-8 — T1-2 실측) · 소거 1 · **잔여 존치 0**
> 🔴 **존치 5건 상태 갱신(T1-2 독립 검증 · 2026-08-28 · E1 · 오케 판정 승인)** — 「T0-6 대조 판정」 열은
> **T0-6 기준 판정이므로 손대지 않는다**. 갱신한 것은 **상태** 열뿐이다: 「T0-6이 명시하지 않았다」는 사실은 그대로이고,
> 「Phase 1 데이터가 그 요구를 채웠는가」만 바뀌었다. 둘을 한 칸에 섞으면 나중에 T0-6 개정 이력을 읽을 수 없다.
> 🔴 **D-8은 연결 좌표가 v0.2 기재(`WO-2026-0113`·`INC-2026-014`)와 다르다** — 2025년 정비가 2026년 작업지시서에서
> 나올 수 없어(시간 역행) `MR-2025-0087 → WO-2025-0087 → INC-2025-019`로 정정했다. 문항 본문도 함께 고쳤다(`Q-MULTIHOP-002`).
> ◻ **충족은 「지금 그렇다」는 실측이지 「앞으로도 그렇다」는 보증이 아니다** — 생성기가 바뀌면 조용히 무너지는 종류다.
> 그래서 5건 전부를 **재실행 가능한 표본으로 고정**했다: `tests/data/seed-integrity.sql`(C-3·C-4=D-2 · C-5=D-3 · C-6·C-7=D-5 · C-8=D-6 · C-15=D-8).
> 근거: `evidence/t1-2-seed-verification.md` · 재현 = `pwsh tests/data/run-seed-integrity.ps1`

> **존치했던 5건(D-2·D-3·D-5·D-6·D-8)은 전부 「T0-6이 틀렸다」가 아니라 「스펙 입도 아래의 데이터 생성 요구」다.** T0-6 결함으로 계수하지 않는다 — Phase 1 synthetic data 생성 시 충족해야 할 조건이다.
...
| # | 미결 | 해소 시점 | 담당 |
|---|---|---|---|
| ~~U-1~~ | ✅ **닫음(2026-08-29 · 색인 착지 후 검증 좌석 실측)** — `chunk_id` 바인딩 완료. 결과·판정은 **§8**. 🔴 「바인딩 가능」과 「chunk 입도가 더 말해준다」는 다르다 — 후자는 이 코퍼스에서 **거짓**으로 실측됐다(§8.2) | 닫음 | — |
...
| ~~U-7~~ | ✅ **닫음(2026-08-29 · 오케 승인 후 집행)** — `Q-SAFETY-001` 앵커 자리표시자 4종을 실값으로 확정했다: `DOC-SAF-0029`·`DOC-SAF-0030`·`DOC-SAF-0029@r3`·`DOC-SAF-0030@r3`·`SAF-PPE-01`. 근거 = 검증 좌석 파일 실측 ↔ 구현 좌석 DB 재도출의 **독립 교차 일치**(E1). ◇ 표기는 존치(§0.5 — T0-6 미명시라는 사실은 불변) | 닫음 | — |
...
1. ~~**◇ 5건이 여전히 E4(가설)** — Phase 1 데이터 생성이 §5 존치 요구를 충족하지 못하면 `Q-DIRECT-002`·`Q-DIRECT-003`·`Q-MULTIHOP-002`·`Q-MULTIHOP-003`·`Q-SAFETY-001` 5문이 측정 불가가 된다. v0.1의 ◇ 10건에서 **절반으로 줄었으나 0은 아니다.**~~
   🔴 **해소 — 2026-08-29 정정(T1-7 A단 ① · E1 · 오케 승인분)**. 원문은 「Phase 1 데이터가 §5 존치 요구를 충족하지 **못하면**」이라는 조건절이었다. §5가 D-2·D-3·D-5·D-6·D-8 **전건 충족(E1 · T1-2)**으로 갱신되면서 그 조건이 풀렸는데 이 절만 남아, **같은 파일이 §5에서는 「잔여 존치 0」을·§7에서는 「여전히 E4」를** 말하고 있었다. 원문은 drift 기록으로 남기고 취소선만 긋는다(§0.2 원칙).
   - **지금 상태**: 5문 전부 측정 가능. 각 문항의 「◇ 의존」 주석에 충족 근거와 **상설 그물**(C-3·C-4·C-5·C-6·C-7·C-8·C-15)을 병기했다. 재현 = `pwsh tests/data/run-seed-integrity.ps1` — 2026-08-29 자기 실행 **22/22 PASS · exit 0**(E1).
   - ~~🔴 **그래도 「0」이라고 쓰지 않는다.** 남은 것은 성질이 다르다: `Q-SAFETY-001`의 앵커가 아직 자리표시자다~~ → **2026-08-29 닫음**(U-7 · 오케 승인 후 실값 확정 집행). 앵커 4종 치환 완료.
   - 🔴 **그래도 여전히 「전부 측정됐다」가 아니다.** 남은 것은 또 성질이 다르다: **chunk 입도 evidence가 미측정**(U-1)이고, 화면이 박은 chunk 좌표 정합(V-1)은 **화면 쪽 결함 확정으로 오케 패스에 잠겨 있다**. 「데이터 요구 미충족」·「앵커 미확정」·「미측정」을 한 칸에 섞으면 다음 사람이 또 stale을 읽는다.
2. ~~**chunk 입도 evidence 미확정**(U-1) — Recall@K·nDCG@K를 chunk 입도로 계산하려면 색인 후 재바인딩이 필요하다. 현재는 **문서·revision 입도까지만 판정 가능**하다.~~
   🔴 **2026-08-29 해소 — 다만 결론이 바뀌지 않았고, 바뀌지 않은 «이유»가 다르다**(§8). 색인이 착지해 재바인딩은 **가능해졌고 실제로 했다**(14좌표 · 전건 유일 chunk). 그런데 실측해 보니 **45 approved revision 중 42개가 chunk 1개**라, chunk 입도 지표가 문서 입도와 거의 같은 것을 잰다. 즉 「아직 못 한다」가 아니라 **「해 봤더니 이 코퍼스에서는 변별하지 못한다」**다. 정본 입도는 문서·revision으로 유지하고, chunk 좌표는 **인용 추적** 용도로 쓴다.
...
## 8. chunk 입도 evidence 바인딩 (U-1 해소 · 2026-08-29 · E1)

> 🔴 **측정-주장 경계(§0.1) 그대로**: 이 절은 «정답 근거의 **좌표**»를 확정한 것이지 «검색 **성능**»을
> 잰 것이 아니다. §3.1·§3.2의 `Actual`은 **전량 `Not measured`로 그대로 둔다.** 색인이 생겼다고
> 성능이 측정된 것이 아니다.

**출처**: 검증 좌석 격리 스택의 **재빌드 색인 실물**(chunk 59 / approved revision 45 ·
`chunking_policy_version=1` · `section_sentence`/512/overlap 0 · `intfloat/multilingual-e5-small` 384d).

### 8.1 바인딩 표 — 세 겹으로 판정했다

| 판정 축 | 무엇을 보는가 |
|---|---|
| 존재 | 그 `chunk_id`가 색인에 실재하는가 |
| 정방향 | 그 chunk 본문이 앵커 문구를 담는가 |
| **역방향** | 같은 revision에서 그 문구를 담은 chunk가 **정확히 1개**인가 |

🔴 역방향이 없으면 「여러 후보 중 하나」를 정답이라 부르게 된다 — 그 순간 좌표는 판정 기준이 아니라 취향이다.

| 문항 | chunk 좌표 | 앵커 문구 | 존재·정방향·역방향 |
|---|---|---|---|
| `Q-DIRECT-002` | `DOC-SOP-0014@r2#001` | `### 3.4 필요 공구 및 자재` · `## 4. 예상 작업 시간` | ✅ 3/3 (각) |
| `Q-DIRECT-003` | `DOC-SOP-0014@r2#001` | `### 3.2 진단 기준` | ✅ 3/3 |
| `Q-MULTIHOP-001` | `DOC-SOP-0014@r2#001` · `DOC-SAF-0029@r3#000` | 「진동 RMS가 기준치의 150%…」 · 「전원 차단 후 잠금·표시(LOTO) 시행」 | ✅ 3/3 (각) |
| `Q-MULTIHOP-002` | `DOC-MRP-0087@r1#000` | 「베어링 교체」 · 「2025-02-11」 | ✅ 3/3 (각) |
| `Q-SAFETY-001` | `DOC-SOP-0014@r2#001` · `DOC-SAF-0029@r3#000` · `DOC-SAF-0030@r3#000` | `### 3.3 필요 부품` · LOTO · 「보호장갑·보안경」 | ✅ 3/3 (각) |
| `Q-DIRECT-001` | **없음** | — | 🔴 **정답 근거에 문서가 없다**(§2) — 없는 것이 정답이다 |
| `Q-UNANS-001` | **없음** | 비용 표현 | ✅ chunk 본문 전수 스캔 **적중 0** |
| `Q-UNANS-002` | **없음** | `EQ-CNC-999` | ✅ chunk 본문 전수 스캔 **적중 0** |

`Q-MULTIHOP-003`·`Q-SAFETY-002`는 정답 근거가 graph·구조화 속성 쪽이라 chunk 앵커를 두지 않는다.

**상설 그물**: `tests/data/eval-chunk-binding.sql` (+ `run-eval-chunk-binding.ps1`) — 위 표를 매 실행
재대조한다. 🔴 **표만 두고 그물을 두지 않으면 재색인·정책 개정이 좌표를 조용히 옮겨도 아무도 모른다.**
2026-08-29 실측 **15/15 PASS · exit 0**.

### 8.2 🔴 그래서 chunk 입도 지표를 쓸 것인가 — **아니다. 이 코퍼스에서는 변별하지 못한다**

| 실측(E1) | 값 |
|---|---:|
| approved revision 총수 | 45 |
| 그중 **chunk가 1개뿐인** revision | **42** |
| chunk가 2개 이상인 revision | 3 (`@r2#000~#002` 3 · `DOC-MAN-0022@r1` 6 · `DOC-MAN-0021@r1` 8) |
| 문항별 정답 chunk 수 `|relevant|` | **1 ~ 3** |

- **Recall@K**: 42/45 문서에서 chunk = 문서다. K≥3이면 문서를 맞힌 검색기는 chunk도 전부 맞힌다 —
  **K와 무관하게 포화**한다. 전략 간 차이를 만들지 못한다.
- **nDCG@K**: `|relevant|`가 1인 문항이 다수라 이상 DCG가 단일 항이 되어 **사실상 역순위(MRR)로 퇴화**한다.
- 즉 **「chunk 입도로 계산할 수 있다」와 「chunk 입도가 문서 입도보다 더 말해준다」는 다르다.**
  전자는 참이 됐고, 후자는 **거짓으로 실측됐다.**

**권고(검증 좌석 소견 · E3)**
1. D3 비교의 **정본 입도는 문서·revision으로 유지**한다. chunk 좌표는 **인용 추적(citation validity)**과
   화면 좌표 결속에 쓴다 — 그 용도에서는 §8.1대로 이미 작동한다.
2. chunk 입도 지표를 «되살리려고» 정책을 건드리지 않는다. `chunking_policy_version=1`은 동결이고,
   화면 앵커(V-1)가 그 위에 결속돼 있다. **지표를 위해 정책을 역산하는 것이 정확히 인계 ⑴이 금지한 것**이다.
3. 코퍼스가 커지면(문서가 길어지거나 수가 늘면) 이 축은 저절로 되살아난다. 되살아난 시점은
   `tests/data/eval-chunk-binding.sql`의 **B-99**(chunk 1개뿐인 revision 수)가 **FAIL로 알려 준다** —
   그 수가 흔들리면 §8.2의 포화 판정을 다시 써야 한다는 뜻이다.
