...
| S8 | 🔴 **develop 개선 트랙**(폐하 하명 09-03 18:21): 디자인 개선 T7-* — 현 상태 «측정» → **설계방향 보고·컨펌** → **시안 컨펌** → 구현·검증 → **develop PR** | S7 | develop 검증 PASS. 🔴 **main 반영은 폐하가 직접 판단한다**(「develop 검증후 메인 반영하겠다」 · 「현시점 메인 배포는 건들지 않는다」) · 다음 입력 = 폐하께서 09-04 배포물을 직접 쓰시고 주시는 개선사항(18:22) |
...
| 3 | Always-on Sandbox UX | S5~S6 | ⏳ 진행 — 6티켓(T3-1~T3-6 · 분모 31→37) · 진입 재가 08-30 15:58 · 🔴 **Feature Freeze 조기 선언 08-31 09:57**(폐하 재가 ⓑ — T3-4·T3-5 «최소 형상» · 시각화·툴팁 확장·폴리시 축은 §3 배포 후 트랙) |
| 4 | Live AI 연결(Tunnel·streaming·fallback) | S7 | ⏳ 진입 재가 08-31 09:57 — 4티켓(T4-1~T4-4 · 분모 37→46) · P3 잔여와 «병행» · 결정 ①~⑤ 승인(Tunnel = Funnel→Named · Vercel = 착수 시 확인 1회 · 동등 보호 · Cloud DB 미사용 · `.github/infra` = 구현 scope) · 🔴 **폐하 하명 08-31 23:25 「P3 완료되면 P4 자동 승인」** = Phase 관문 재가 불요(착수·완결 보고만) · 공개 행위 확인 1회(주법 §5)는 보존 · T4-3·T4-4 티켓 선작성 23:25 |
| 5 | 평가·보안·운영(Gate 1~8 전건) | 배포 후 | 등재 — §3 별도 재가 08-31 09:57 · 5티켓(T5-1~T5-5) · 착수 = P4 흐름 보며(T5-1 즉시 병행 가능) · 🔴 **폐하 하명 08-31 23:25 「P5 까지 자동 승인」** = Phase 관문 재가 불요(착수·완결 보고만) · 티켓 T5-1~T5-5 선작성 = 오케 다음 유휴 슬롯 |
| 6 | 🔴 **P0 확장(폐하 결정분)** — Live 합성·소유자 스위치·체감 속도·**디자인 전면**·가이드 투어 + 포트폴리오 패키징(영상·README·Release) | 배포 후 | 🔴 **T6-1~T6-5 완결 09-03**(48/54) · 승격 6회차 공개면 반영·외부 도달 확정(main `1987ab3`) · 패키징(영상·Release checklist)은 **대기** |
| 7 | ✅ **완결 09-03 20:49 — 디자인 개선 2티켓 전건**(T7-1 접근성 설정 대응 · T7-2 브라우저 호환 축 · 원장 **50/54**) — 처방 5분해 착지 후 **독립 검증 4축**(44px 성립 · 정렬 **Δ 0** · 회귀 3종 무변 · **가로 넘침 회귀 검출→수리→재측 PASS**) · 판정문 `docs/design/t7-1-accessibility-verdict.md` · 🔴 **잔여는 티켓이 아니라 «결함 원장»에서 관리**(D-30·D-31 = **폐하 결정** · D-32·D-34·D-35·D-36) · 🔴 **main 무접촉 전 구간 유지** · **이전 상태** = 🔴 **진행 09-03 18:21 폐하 하명** · 측정 발주 18:23(좌석 2본) · 🔴 **PR = develop 만 · main 머지 금지**(폐하 게이트) · 설계방향·시안 **컨펌 선행** · 🔴 T6-4 「완결」의 참인 범위 = 크롬 계열 × 4뷰포트 × 2테마(측정-주장 §0.2 정정) |
